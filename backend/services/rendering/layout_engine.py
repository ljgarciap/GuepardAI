...
from providers.llm_provider import generate_json, get_system_config
import models
...
def apply_design_policy(content_manifest: dict, brand_dna, brand_essence=None, job_id=None, db=None) -> dict:
    ...
    policy = parse_essence_to_policy(
        ...
        source_pptx_path=pptx_source_path,
        force_width=getattr(brand_dna, "slide_width_inches", None) or None,
        force_height=getattr(brand_dna, "slide_height_inches", None) or None
    )
...
def generate_presentation_flow(db: Session, job_id: int, req_data: dict):
    ...
    try:
        ...
        if req_data.get("skip_design"):
            print("  [Flow] Skipping Art Direction, building final design from DB...")
            from schemas.presentation import DesignManifest, DesignManifestSlide
            d_slides = []
            saved_slides = db.query(models.PresentationSlide).filter(models.PresentationSlide.job_id == job_id).order_by(models.PresentationSlide.slide_number.asc()).all()
            for s in saved_slides:
                primary_path = None
            ...
            final_design = DesignManifest(job_id=job_id, slides=d_slides, theme={})
        else:
