...
from providers.llm_provider import generate_vision_json, generate_json
...
def extract_pdf_dna(file_path: str, source_filename: str,
    ...
    # --- 2. BULLETPROOF XREF EXTRACTION FOR IMAGES ---
    print(f"  [DNA] Scanning all PDF XREFs for deeply nested media...", flush=True)
...
def extract_pptx_dna(file_path: str, source_filename: str,
    ...
    # --- 0. BULLETPROOF ZIP EXTRACTION ---
    # Captura absolutamente todo (imágenes anidadas, SmartArt, backgrounds) que python-pptx ignora
    print(f"  [DNA] Scanning Zip package for deeply nested media...", flush=True)
    import zipfile
    try:
        ...
        print(f"  [DNA] Zip extraction failed: {e}", flush=True)
