...
- Claves nuevas `tm_group_max_depth`, `tm_empty_rewrite_policy`, `tm_outline_enabled`, `tm_outline_rag_k`, `tm_outline_context_max_chars` (seeds Fase 1-2), `tm_char_width_factor`, `tm_line_height_factor`, `tm_fill_safety_factor`, `tm_fitcheck_max_retries`, `tm_reset_autofit` (seeds Fase 3) y `tm_visual_qa_enabled` (**default off** — gasta tokens Vision), `tm_visual_qa_max_slides` (seeds Fase 4, 2026-07-08). Para activar el QA visual: `UPDATE system_configs SET value='true' WHERE key='tm_visual_qa_enabled';`
